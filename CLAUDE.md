...
- **Checklists:** `checkCommand` (toolbar ☑ — starts a checklist, or toggles the list under the caret between bullets/checkboxes), `rollChecks` (run on every editor `input` and on `select`: nested lists under a checklist inherit it unless marked `class="bullets"`; a parent item with checklist children gets `locked` and its `done` is derived from them), `kidLists`/`kidItems` (a child list is either inside the `<li>` or — as Chrome's indent produces — the `<ul>` sibling right after it), `liAtSelection`. The checkbox is the `<li>`'s `::before`; an editor `mousedown` left of the item's box toggles `done` (locked items just toast).
